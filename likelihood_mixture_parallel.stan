functions {
  ...
  real partial_sum(int[] n_slice,
                   int start, int end,
                   vector log_kappa,
                   vector log_kappa_popmean,
                   vector log_kappa_popsd,
                   vector kappa,
                   vector omega,
                   vector mu,
                   real theta,
    ...
    for (H in start:end){
      partial_ll += log_mix(theta,
                            normal_lpdf( log_kappa[H] | log_kappa_popmean[1], log_kappa_popsd[1] ),
                            normal_lpdf( log_kappa[H] | log_kappa_popmean[2], log_kappa_popsd[2] ));
      partial_ll += exponential_lpdf( omega[H] | 1);
      partial_ll += exponential_lpdf( mu[H] | 0.5 );  
...
parameters {
  ...
  vector<lower=0>[2] log_kappa_popmean;
  vector<lower=0>[2] log_kappa_popsd;
  vector<lower=0>[l] log_kappa;
...
model{
  ...
  log_kappa_popmean[1] ~ normal(0,1); 
  log_kappa_popmean[2] ~ normal(0,1); 
  log_kappa_popsd[1] ~ exponential(5);
  log_kappa_popsd[2] ~ exponential(5);
  theta ~ beta(5,5);
    
  target += reduce_sum(partial_sum, n, 
                       grainsize,
                       log_kappa, log_kappa_popmean, log_kappa_popsd, kappa, omega, mu, theta, pi, X);
}