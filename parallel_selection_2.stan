functions {
  ...
  real partial_sum(int[] n_slice,
                   int start, int end,
                   vector kappa,
                   vector kappa_popmean,
                   vector kappa_popsd,
                   vector omega,
                   vector omega_popmean,
                   vector omega_popsd,
                   vector mu,
                   real theta_kappa,
                   real theta_omega,
    ...
    for (H in start:end){
      partial_ll += log_mix(theta_kappa,
                            normal_lpdf( kappa[H] | kappa_popmean[1], kappa_popsd[1] ),
                            normal_lpdf( kappa[H] | kappa_popmean[2], kappa_popsd[2] ));
      partial_ll += log_mix(theta_omega,
                            normal_lpdf( omega[H] | omega_popmean[1], omega_popsd[1] ),
                            normal_lpdf( omega[H] | omega_popmean[2], omega_popsd[2] ));
      ...
      for (A in 1:61){
        ...
        lik_full[A] = lik + lgamma(alpha_A[A]) - lgamma(n_slice[end-start]+alpha_A[A]) + lgamma(n_slice[end-start]+1);
...
parameters {
  ...
  vector<lower=0>[2] kappa_popmean;
  vector<lower=0>[2] kappa_popsd;
  vector<lower=0>[l] kappa;
  
  vector<lower=0>[2] omega_popmean;
  vector<lower=0>[2] omega_popsd;
  vector<lower=0>[l] omega;
  
  real<lower=0, upper=1> theta_kappa;
  real<lower=0, upper=1> theta_omega;
  
  real<lower=0> nu_kappa;
  //real<lower=0> nu_omega;
}
...
model{
  ...
  kappa_popmean ~ exponential(0.5); 
  kappa_popsd ~ student_t(nu_kappa, 0, 1);

  omega_popmean[1] ~ normal(0.5, 0.5); 
  omega_popmean[2] ~ normal(1, 1);
  omega_popsd[1] ~ normal(0, 1);
  omega_popsd[2] ~ normal(0, 2);

  theta_kappa ~ beta(3, 3);
  theta_omega ~ beta(9, 1.5);
  
  nu_kappa ~ gamma(3, 1);
  //nu_omega ~ gamma(2, 0.1);
    
  target += reduce_sum(partial_sum, n, 
                       grainsize,
                       kappa, kappa_popmean, kappa_popsd, 
                       omega, omega_popmean, omega_popsd, 
                       mu, theta_kappa, theta_omega, pi, X);
}