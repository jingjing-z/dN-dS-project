...
m1.1 <- map2stan(
  alist(
    # eigen decomposition
    RM <- PDRM(mu, kappa, omega),
    V <- eigen(RM)$vectors,
    D <- diag(eigen(RM)$values), 
    
    # x_j: the number of times allele j was counted
    x <- rep(NA, 61),
    ...
    for (i in 1:61){
      ...
      m_AA <- 0
      for (k in 1:61){
        m_Ai <- m_Ai + (V[A,k]*(V[k,i])^(-1))/(1-D[k,k])
        m_AA <- m_AA + (V[A,k]*(V[k,A])^(-1))/(1-D[k,k])
      }
      ...
      # equation 5&6
      lik <- lik + pi[i]*( lgamma(alpha0) - lgamma(n+alpha0) +
                             dlgamma(x[i]+alpha_A[i]) - dlgmma(alpha_A[i]) +
                             lgamma(n+1) - lgamma(x[i]+1) ) #this line: n choose vector x 
