functions {
  ...
  real partial_sum(int[] n_slice,
    ...
    real partial_ll;
     ...
     partial_ll += lognormal_lpdf( kappa | 1, 1.5);
     partial_ll += exponential_lpdf( omega | 0.5);
     partial_ll += exponential_lpdf( mu | 0.5 );  
     ...
     for (A in 1:61){
       ...
       lik_full[A] = lik + lgamma(alpha_A[A]) - lgamma(n_slice+alpha_A[A]) + lgamma(n_slice+1);
       }
